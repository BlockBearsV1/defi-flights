    ...
    for (let i = 0; i < contents.length; i++) {
      ...
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await watchTransactions.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
            const quantityPerReward = ethers.ethers.utils.formatUnits(amount, tokenMetadata.decimals);
            const totalRewards = ethers.ethers.utils.formatUnits(ethers.BigNumber.from(reward.totalAmount).div(amount), tokenMetadata.decimals);
            erc20Rewards.push({
              contractAddress: reward.assetContract,
              quantityPerReward,
              totalRewards
