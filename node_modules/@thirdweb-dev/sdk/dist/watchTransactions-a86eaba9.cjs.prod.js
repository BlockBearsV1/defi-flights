...
const EnsSchema = zod.z.custom(ens => typeof ens === "string" && (ens.endsWith(".eth") || ens.endsWith(".cb.id"))).transform(async ens => resolveEns(ens)).refine(address => !!address && ethers.utils.isAddress(address), {
...
const AddressSchema = zod.z.custom(address => typeof address === "string" && ethers.utils.isAddress(address), out => {
  return {
...
const AddressOrEnsSchema = zod.z.union([AddressSchema, EnsSchema], {
  invalid_type_error: "Provided value was not a valid address or ENS name"
});
const RawDateSchema = zod.z.union([zod.z.date().transform(i => {
...
const SDKOptionsSchema = zod.z.object({
  ...
  thirdwebApiKey: zod.z.string().default(QueryParams.DEFAULT_API_KEY),
  alchemyApiKey: zod.z.string().optional(),
  infuraApiKey: zod.z.string().optional(),
...
const SnapshotEntryInput = zod.z.object({
  ...
  currencyAddress: AddressOrEnsSchema.default(ethers.ethers.constants.AddressZero).optional() // defaults to AddressZero, but can be undefined for old snapshots
});

/**
 * @internal
 */
const SnapshotInputSchema = zod.z.union([zod.z.array(zod.z.string()).transform(async strings => await Promise.all(strings.map(address => SnapshotEntryInput.parseAsync({
  address
...
const BaseSignaturePayloadInput = zod.z.object({
  to: AddressOrEnsSchema.refine(address => address.toLowerCase() !== ethers.constants.AddressZero, {
  ...
  primarySaleRecipient: AddressOrEnsSchema.default(ethers.constants.AddressZero)
});
...
function getChainProvider(network, sdkOptions) {
  ...
  if (!rpcUrl) {
    rpcUrl = `https://${chainId || network}.rpc.thirdweb.com/${options.thirdwebApiKey || QueryParams.DEFAULT_API_KEY}`;
  }
  if (!rpcUrl) {
...
class ContractWrapper extends RPCConnectionHandler {
  ...
  async call(functionName) {
    ...
    let txOptions = overrides ? await CallOverrideSchema.parseAsync(overrides) : undefined;
...
function toWei(amount) {
  return ethers.utils.parseEther(QueryParams.AmountSchema.parse(amount));
}
...
async function resolveContractUriFromAddress(address, provider) {
  ...
  try {
    ...
    if (utils.isAddress(implementationAddress) && implementationAddress !== ethers.ethers.constants.AddressZero) {
      return await resolveContractUriFromAddress(implementationAddress, provider);
    }
  } catch (e) {
    // ignore
...
const ERROR_SUBSTRINGS = ["eip-155", "protected", "invalid chain id for signer", "chain id none", "chain_id mismatch", "recovered sender mismatch", "transaction hash mismatch", "chainid no support"];
...
function getSaltHash(bytecode) {
  const bytecodeHash = ethers.ethers.utils.id(bytecode);
  const salt = `tw.${bytecodeHash}`;
  const saltHash = ethers.ethers.utils.id(salt);
  return saltHash;
}
 ...
 */
function getInitBytecodeWithSalt(bytecode, encodedArgs) {
  const saltHash = getSaltHash(bytecode);
  const initBytecodeWithSalt = ethers.ethers.utils.solidityPack(["bytes32", "bytes", "bytes"], [saltHash, bytecode, encodedArgs]);
 ...
 */
function computeDeploymentAddress(bytecode, encodedArgs, create2FactoryAddress) {
  const saltHash = getSaltHash(bytecode);

  // 1. create init bytecode hash with contract's bytecode and encoded args
  const initBytecode = ethers.ethers.utils.solidityPack(["bytes", "bytes"], [bytecode, encodedArgs]);

  // 2. abi-encode pack the deployer address, salt, and bytecode hash
  const deployInfoPacked = ethers.ethers.utils.solidityPack(["bytes1", "address", "bytes32", "bytes32"], ["0xff", create2FactoryAddress, saltHash, ethers.ethers.utils.solidityKeccak256(["bytes"], [initBytecode])]);

  // 3. hash the packed deploy info
  const hashedDeployInfo = ethers.ethers.utils.solidityKeccak256(["bytes"], [deployInfoPacked]);
 ...
 */
async function computeEOAForwarderAddress(provider, storage, create2Factory) {
  if (!create2Factory) {
 ...
 * @param create2Factory
 */
async function computeForwarderAddress(provider, storage, create2Factory) {
  if (!create2Factory) {
    create2Factory = await getCreate2FactoryAddress(provider);
  }
  return (await computeDeploymentInfo("infra", provider, storage, create2Factory, {
...
function getKeylessTxn(transaction, signature) {
  ...
  const digest = ethers.ethers.utils.arrayify(ethers.ethers.utils.keccak256(ethers.ethers.utils.serializeTransaction(transaction)));

  // 2. Determine signer address from custom signature + txn
  const signer = ethers.ethers.utils.recoverAddress(digest, signature);

  // 3. Create the signed serialized txn string.
  // To be sent directly to the chain using a provider.
  const signedSerializedTx = ethers.ethers.utils.serializeTransaction(transaction, signature);
  return {
    signer: signer,
...
async function deployCreate2Factory(signer, options) {
  ...
  if (!factoryExists) {
    ...
    const valueToSend = CUSTOM_GAS_FOR_CHAIN[networkId] ? ethers.BigNumber.from(CUSTOM_GAS_FOR_CHAIN[networkId].gasPrice).mul(100000) : toWei("0.01");
...
async function deployWithThrowawayDeployer(signer, transactions, options) {
  ...
  const deployTxns = await Promise.all(transactionBatches.map(txBatch => {
    ...
    const deployer = new ethers.ethers.ContractFactory(DEPLOYER_ABI, DEPLOYER_BYTECODE).connect(signer).deploy(txBatch);
    return deployer;
  }));
...
async function computeDeploymentInfo(contractType, provider, storage, create2Factory, contractOptions) {
  ...
  const encodedArgs = await encodeConstructorParamsForImplementation(metadata, provider, storage, create2Factory, contractOptions?.constructorParams);
 ...
 */
async function encodeConstructorParamsForImplementation(compilerMetadata, provider, storage, create2Factory, constructorParamMap) {
  ...
  let constructorParamTypes = constructorParams.map(p => {
    if (p.type === "tuple[]") {
      return ethers.ethers.utils.ParamType.from(p);
    } else {
      return p.type;
    }
  });
  const constructorParamValues = await Promise.all(constructorParams.map(async p => {
    ...
    if (p.name && p.name.includes("nativeTokenWrapper")) {
      ...
      if (nativeTokenWrapperAddress === ethers.ethers.constants.AddressZero) {
      ...
      if (compilerMetadata.analytics?.contract_name && compilerMetadata.analytics.contract_name === "Pack") {
  ...
  const encodedArgs = ethers.ethers.utils.defaultAbiCoder.encode(constructorParamTypes, constructorParamValues);
  return encodedArgs;
}
...
function getCreate2FactoryDeploymentInfo(chainId, gasPrice) {
  const signature = ethers.ethers.utils.joinSignature(SIGNATURE);
  ...
  const create2FactoryAddress = ethers.ethers.utils.getContractAddress({
...
async function fetchAndCachePublishedContractURI(contractName) {
  ...
  const publishedContract = await new ThirdwebSDK("polygon").getPublisher().getVersion(THIRDWEB_DEPLOYER$1, contractName);
...
function estimateGasForDeploy(initCode) {
  let gasLimit = ethers.ethers.utils.arrayify(initCode).map(x => x === 0 ? 4 : 16).reduce((sum, x) => sum + x) + 200 * initCode.length / 2 + 6 * Math.ceil(initCode.length / 64) + 32000 + 21000;
...
function generatePluginFunctions(pluginAddress, pluginAbi) {
  const pluginInterface = new ethers.ethers.utils.Interface(pluginAbi);
...
class TransactionContext {
  constructor(options) {
    ...
    QueryParams._defineProperty(this, "signer", void 0);
    QueryParams._defineProperty(this, "storage", void 0);
    QueryParams._defineProperty(this, "gasMultiple", void 0);
  ...
  async send() {
    ...
    if (!overrides.gasLimit) {
      ...
      try {
        ...
        const abi = JSON.parse(this.contract.interface.format(utils_js.FormatTypes.json));
...
class DeployTransaction extends TransactionContext {
  constructor(options) {
    super(options);
    QueryParams._defineProperty(this, "factory", void 0);
...
class ContractMetadata {
  constructor(contractWrapper, schema, storage) {
     ...
     */
    QueryParams._defineProperty(this, "set", buildTransactionFunction(async metadata => {
     ...
     * @twfeature ContractMetadata
     * */
    QueryParams._defineProperty(this, "update", buildTransactionFunction(async metadata => {
...
class ContractRoles {
  ...
  constructor(contractWrapper, _roles) {
     ...
     *
     * */
    QueryParams._defineProperty(this, "setAll", buildTransactionFunction(async rolesWithAddresses => {
     ...
     * @twfeature Permissions
     */
    QueryParams._defineProperty(this, "grant", buildTransactionFunction(async (role, address) => {
     ...
     * @twfeature Permissions
     */
    QueryParams._defineProperty(this, "revoke", buildTransactionFunction(async (role, address) => {
      invariant__default["default"](this.roles.includes(role), `this contract does not support the "${role}" role`);
      const resolvedAddress = await resolveAddress(address);
...
class ContractRoyalty {
  constructor(contractWrapper, metadata) {
     ...
     * @twfeature Royalty
     */
    QueryParams._defineProperty(this, "setDefaultRoyaltyInfo", buildTransactionFunction(async royaltyData => {
     ...
     * @twfeature Royalty
     */
    QueryParams._defineProperty(this, "setTokenRoyaltyInfo", buildTransactionFunction(async (tokenId, royaltyData) => {
...
class ContractPrimarySale {
  constructor(contractWrapper) {
     ...
     */
    QueryParams._defineProperty(this, "setRecipient", buildTransactionFunction(async recipient => {
      return Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
...
class DelayedReveal {
  constructor(contractWrapper, storage, featureName, nextTokenIdToMintFn) {
     ...
     */
    QueryParams._defineProperty(this, "createDelayedRevealBatch", buildTransactionFunction(async (placeholder, metadatas, password, options) => {
      if (!password) {
     ...
     */
    QueryParams._defineProperty(this, "reveal", buildTransactionFunction(async (batchId, password) => {
      if (!password) {
        throw new Error("Password is required");
      }
...
const TieredDropPayloadSchema = BaseSignaturePayloadInput.extend({
  ...
  royaltyRecipient: AddressOrEnsSchema.default(ethers.constants.AddressZero),
  royaltyBps: QueryParams.BasisPointsSchema.default(0),
  quantity: BigNumberSchema.default(1)
});
...
class Erc721TieredDrop {
  constructor(erc721, contractWrapper, storage) {
    ...
    QueryParams._defineProperty(this, "erc721", void 0);
    QueryParams._defineProperty(this, "storage", void 0);
    QueryParams._defineProperty(this, "createBatchWithTier", buildTransactionFunction(async (metadatas, tier, options) => {
      ...
      return Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "lazyMint",
        args: [batch.length, baseUri.endsWith("/") ? baseUri : `${baseUri}/`, tier, ethers.ethers.utils.toUtf8Bytes("")],
        parse: receipt => {
          const event = this.contractWrapper.parseLogs("TokensLazyMinted", receipt?.logs);
    ...
    QueryParams._defineProperty(this, "createDelayedRevealBatchWithTier", buildTransactionFunction(async (placeholder, metadatas, password, tier, options) => {
      if (!password) {
        throw new Error("Password is required");
      }
      const placeholderUris = await this.storage.uploadBatch([QueryParams.CommonNFTInput.parse(placeholder)], {
...
class Erc721 {
  ...
  constructor(contractWrapper, storage, chainId) {
    ...
    QueryParams._defineProperty(this, "claimCustom", void 0);
    QueryParams._defineProperty(this, "contractWrapper", void 0);
    QueryParams._defineProperty(this, "storage", void 0);
    QueryParams._defineProperty(this, "_chainId", void 0);
    /**
 ...
 * @internal
 */
const AirdropInputSchema = zod.z.union([zod.z.array(zod.z.string()).transform(async strings => await Promise.all(strings.map(address => AirdropAddressInput.parseAsync({
  address
...
class Erc1155BatchMintable {
  constructor(erc1155, contractWrapper, storage) {
    ...
    QueryParams._defineProperty(this, "to", buildTransactionFunction(async (to, metadataWithSupply) => {
      ...
      const uris = await uploadOrExtractURIs(metadatas, this.storage);
      const resolvedAddress = await resolveAddress(to);
      const encoded = await Promise.all(uris.map(async (uri, index) => this.contractWrapper.readContract.interface.encodeFunctionData("mintTo", [resolvedAddress, ethers.ethers.constants.MaxUint256, uri, supplies[index]])));
      return Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "multicall",
        args: [encoded],
...
class Erc1155Mintable {
  ...
  async getMintTransaction(to, metadataWithSupply) {
    ...
    return Transaction.fromContractWrapper({
      contractWrapper: this.contractWrapper,
      method: "mintTo",
      args: [await resolveAddress(to), ethers.ethers.constants.MaxUint256, uri, metadataWithSupply.supply]
    });
  }
...
class MarketplaceV3DirectListings {
  ...
  constructor(contractWrapper, storage) {
     ...
     */
    QueryParams._defineProperty(this, "createListing", buildTransactionFunction(async listing => {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createListingsBatch", buildTransactionFunction(async listings => {
     ...
     * @twfeature DirectListings
     */
    QueryParams._defineProperty(this, "updateListing", buildTransactionFunction(async (listingId, listing) => {
      const parsedListing = await DirectListingInputParamsSchema.parseAsync(listing);
      await handleTokenApproval(this.contractWrapper, this.getAddress(), parsedListing.assetContractAddress, parsedListing.tokenId, await this.contractWrapper.getSignerAddress());
      const normalizedPricePerToken = await normalizePriceValue(this.contractWrapper.getProvider(), parsedListing.pricePerToken, parsedListing.currencyContractAddress);
      return Transaction.fromContractWrapper({
     ...
     * @twfeature DirectListings
     */
    QueryParams._defineProperty(this, "cancelListing", buildTransactionFunction(async listingId => {
      return Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
     ...
     * @twfeature DirectListings
     */
    QueryParams._defineProperty(this, "buyFromListing", buildTransactionFunction(async (listingId, quantityDesired, receiver) => {
      if (receiver) {
     ...
     * @twfeature DirectListings
     */
    QueryParams._defineProperty(this, "approveBuyerForReservedListing", buildTransactionFunction(async (listingId, buyer) => {
     ...
     * @param buyer - Address of buyer being approved
     */
    QueryParams._defineProperty(this, "revokeBuyerApprovalForReservedListing", buildTransactionFunction(async (listingId, buyer) => {
      const isApproved = await this.isBuyerApprovedForListing(listingId, buyer);
      if (isApproved) {
        return Transaction.fromContractWrapper({
          contractWrapper: this.contractWrapper,
     ...
     * @twfeature DirectListings
     */
    QueryParams._defineProperty(this, "approveCurrencyForListing", buildTransactionFunction(async (listingId, currencyContractAddress, pricePerTokenInCurrency) => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
     ...
     * @twfeature DirectListings
     */
    QueryParams._defineProperty(this, "revokeCurrencyApprovalForListing", buildTransactionFunction(async (listingId, currencyContractAddress) => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
      const resolvedCurrencyAddress = await resolveAddress(currencyContractAddress);
      if (resolvedCurrencyAddress === listing.currencyContractAddress) {
...
class MarketplaceV3EnglishAuctions {
  ...
  constructor(contractWrapper, storage) {
     ...
     */
    QueryParams._defineProperty(this, "createAuction", buildTransactionFunction(async auction => {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createAuctionsBatch", buildTransactionFunction(async listings => {
      const data = await Promise.all(listings.map(async listing => {
     ...
     * @twfeature EnglishAuctions
     */
    QueryParams._defineProperty(this, "buyoutAuction", buildTransactionFunction(async auctionId => {
      ...
      return this.makeBid.prepare(auctionId, ethers.ethers.utils.formatUnits(auction.buyoutBidAmount, currencyMetadata.decimals));
    }));
    /**
     ...
     * @twfeature EnglishAuctions
     */
    QueryParams._defineProperty(this, "makeBid", buildTransactionFunction(async (auctionId, bidAmount) => {
      const auction = await this.validateAuction(ethers.BigNumber.from(auctionId));
     ...
     * @twfeature EnglishAuctions
     */
    QueryParams._defineProperty(this, "cancelAuction", buildTransactionFunction(async auctionId => {
      const winningBid = await this.getWinningBid(auctionId);
      if (winningBid) {
     ...
     * @twfeature EnglishAuctions
     */
    QueryParams._defineProperty(this, "closeAuctionForBidder", buildTransactionFunction(async (auctionId, closeFor) => {
      if (!closeFor) {
     ...
     * @twfeature EnglishAuctions
     */
    QueryParams._defineProperty(this, "closeAuctionForSeller", buildTransactionFunction(async auctionId => {
      const auction = await this.validateAuction(ethers.BigNumber.from(auctionId));
      try {
        return Transaction.fromContractWrapper({
          contractWrapper: this.contractWrapper,
     ...
     * @twfeature EnglishAuctions
     */
    QueryParams._defineProperty(this, "executeSale", buildTransactionFunction(async auctionId => {
      const auction = await this.validateAuction(ethers.BigNumber.from(auctionId));
      try {
        const winningBid = await this.getWinningBid(auctionId);
...
class MarketplaceV3Offers {
  ...
  constructor(contractWrapper, storage) {
     ...
     * @twfeature Offers
     */
    QueryParams._defineProperty(this, "makeOffer", buildTransactionFunction(async offer => {
     ...
     * @twfeature Offers
     */
    QueryParams._defineProperty(this, "cancelOffer", buildTransactionFunction(async offerId => {
      return Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "cancelOffer",
        args: [offerId]
     ...
     * @twfeature Offers
     */
    QueryParams._defineProperty(this, "acceptOffer", buildTransactionFunction(async offerId => {
...
const EditionDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract, _network] = await Promise.all([await EditionDropInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./edition-drop-54718bc6.cjs.prod.js'); }), provider.getNetwork()]);
...
const EditionInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([EditionInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./edition-f6ffb3fd.cjs.prod.js'); }), provider.getNetwork()]);
...
const MarketplaceInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MarketplaceInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./marketplace-cd6adee6.cjs.prod.js'); }), provider.getNetwork()]);
...
const MarketplaceV3Initializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MarketplaceV3Initializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./marketplacev3-259ce669.cjs.prod.js'); }), provider.getNetwork()]);
...
const MultiwrapInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MultiwrapInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./multiwrap-b60f5a3e.cjs.prod.js'); }), provider.getNetwork()]);
...
const NFTCollectionInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTCollectionInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./nft-collection-fc16ef3f.cjs.prod.js'); }), provider.getNetwork()]);
...
const NFTDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTDropInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./nft-drop-f0bfb0ba.cjs.prod.js'); }), provider.getNetwork()]);
...
const PackInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([PackInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./pack-fad0c49e.cjs.prod.js'); }), provider.getNetwork()]);
...
const SignatureDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SignatureDropInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./signature-drop-b69898d8.cjs.prod.js'); }), provider.getNetwork()]);
...
const SplitInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SplitInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./split-88a56caf.cjs.prod.js'); }), provider.getNetwork()]);
...
const TokenDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenDropInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./token-drop-76958524.cjs.prod.js'); }), provider.getNetwork()]);
...
const TokenInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./token-74ea4608.cjs.prod.js'); }), provider.getNetwork()]);
...
const VoteInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([VoteInitializer.getAbi(address, provider, storage), Promise.resolve().then(function () { return require('./vote-ff5fddbf.cjs.prod.js'); }), provider.getNetwork()]);
...
class ContractAppURI {
  constructor(contractWrapper, metadata, storage) {
     ...
     * @twfeature AppURI
     */
    QueryParams._defineProperty(this, "set", buildTransactionFunction(async appURI => {
...
class ContractPlatformFee {
  constructor(contractWrapper) {
     ...
     */
    QueryParams._defineProperty(this, "set", buildTransactionFunction(async platformFeeInfo => {
...
class MultichainRegistry {
  ...
  async getContractAddresses(walletAddress) {
    return (await this.registryLogic.readContract.getAll(await resolveAddress(walletAddress))).filter(result => ethers.utils.isAddress(result.deploymentAddress) && result.deploymentAddress.toLowerCase() !== ethers.constants.AddressZero).map(result => ({
...
class ContractVerifier extends RPCConnectionHandler {
  ...
  async verifyContract(contractAddress, explorerAPIUrl, explorerAPIKey, constructorArgs) {
    ...
    if (constructorArgs) {
      ...
      encodedArgs = ethers.ethers.utils.defaultAbiCoder.encode(paramTypes, paramValues);
    }
...
class ThirdwebSDK extends RPCConnectionHandler {
  ...
  async resolveContractType(contractAddress) {
    try {
      const contract = new ethers.Contract(await resolveAddress(contractAddress), IThirdwebContractABI__default["default"],
...
class ContractFactory extends ContractWrapper {
  ...
  constructor(factoryAddr, network, storage, options) {
    ...
    QueryParams._defineProperty(this, "deploy", buildTransactionFunction(async (contractType, contractMetadata, eventEmitter, version, onExecute) => {
    ...
    QueryParams._defineProperty(this, "deployProxyByImplementation", buildTransactionFunction(async (implementationAddress, implementationAbi, initializerFunction, initializerArgs, eventEmitter, onExecute) => {
      const encodedFunc = ethers.Contract.getInterface(implementationAbi).encodeFunctionData(initializerFunction, initializerArgs);
      const blockNumber = await this.getProvider().getBlockNumber();
      return Transaction.fromContractWrapper({
        contractWrapper: this,
        method: "deployProxyByImplementation",
        args: [implementationAddress, encodedFunc, ethers.ethers.utils.formatBytes32String(blockNumber.toString())],
        parse: receipt => {
          if (onExecute) {
            onExecute();
          }
...
class ContractRegistry extends ContractWrapper {
  ...
  async getContractAddresses(walletAddress) {
    ...
    return (await this.readContract.getAll(await resolveAddress(walletAddress))).filter(adr => ethers.utils.isAddress(adr) && adr.toLowerCase() !== ethers.constants.AddressZero);
  }
}
...
class ContractDeployer extends RPCConnectionHandler {
  ...
  constructor(network, _options, storage) {
     ...
     */
    QueryParams._defineProperty(this, "deployNFTCollection", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployNFTDrop", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deploySignatureDrop", buildDeployTransactionFunction(async metadata => {
     ...
     * @beta
     */
    QueryParams._defineProperty(this, "deployMultiwrap", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployEdition", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployEditionDrop", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployToken", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployTokenDrop", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployMarketplace", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployMarketplaceV3", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployPack", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deploySplit", buildDeployTransactionFunction(async metadata => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployVote", buildDeployTransactionFunction(async metadata => {
     ...
     */
    QueryParams._defineProperty(this, "deployBuiltInContract", buildDeployTransactionFunction(async function (contractType, contractMetadata) {
      let version = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "latest";
      ...
      if (_this.hasLocalFactory()) {
        ...
        return factory.deploy.prepare(contractType, parsedMetadata, _this.events, parsedVersion, () => {
     ...
     */
    QueryParams._defineProperty(this, "deployReleasedContract", buildDeployTransactionFunction(async function (publisherAddress, contractName, constructorParams) {
     ...
     */
    QueryParams._defineProperty(this, "deployViaFactory", buildTransactionFunction(async (factoryAddress, implementationAddress, implementationAbi, initializerFunction, initializerArgs) => {
     ...
     * @param initializerArgs
     */
    QueryParams._defineProperty(this, "deployProxy", buildDeployTransactionFunction(async (implementationAddress, implementationAbi, initializerFunction, initializerArgs) => {
     ...
     * @param options
     */
    QueryParams._defineProperty(this, "deployContractFromUri", buildDeployTransactionFunction(async (publishMetadataUri, constructorParamValues, options) => {
      const signer = this.getSigner();
      invariant__default["default"](signer, "A signer is required");
      const {
        compilerMetadata,
      ...
      if (extendedMetadata && extendedMetadata.factoryDeploymentData && (extendedMetadata.isDeployableViaProxy || extendedMetadata.isDeployableViaFactory) && !forceDirectDeploy) {
        ...
        if (implementationAddress) {
          ...
          if (extendedMetadata.isDeployableViaFactory) {
            ...
            const resolvedFactoryAddress = await resolveAddress(factoryAddress);
            return await this.deployViaFactory.prepare(resolvedFactoryAddress, resolvedImplementationAddress, compilerMetadata.abi, extendedMetadata.factoryDeploymentData.implementationInitializerFunction, paramValues);
          ...
          implementationAddress = deploymentInfo.find(i => i.type === "implementation")?.transaction.predictedAddress;
          ...
          const proxyDeployTransaction = await this.deployViaFactory.prepare(cloneFactory, resolvedImplementationAddress, compilerMetadata.abi, extendedMetadata.factoryDeploymentData.implementationInitializerFunction, paramValues);
     ...
     */
    QueryParams._defineProperty(this, "deployContractWithAbi", buildDeployTransactionFunction(async (abi, bytecode, constructorParams) => {
      const signer = this.getSigner();
      const provider = this.getProvider();
      invariant__default["default"](signer, "Signer is required to deploy contracts");
      const factory = new ethers.ethers.ContractFactory(abi, bytecode).connect(signer);
...
class ContractOwner {
  constructor(contractWrapper) {
     ...
     * @twfeature Ownable
     */
    QueryParams._defineProperty(this, "set", buildTransactionFunction(async address => {
      const resolvedAddress = await resolveAddress(address);
      return Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "setOwner",
...
async function fetchContractMetadataFromAddress(address, provider, storage) {
  ...
  try {
    ...
    console.warn("Failed to get Contract Metadata from IPFS, defaulting to onchain registry", e);
    try {
...
async function verify(contractAddress, chainId, explorerAPIUrl, explorerAPIKey, storage, encodedConstructorArgs) {
  try {
    ...
    const compilerMetadata = await sdk.getPublisher().fetchCompilerMetadataFromAddress(contractAddress);
    ...
    const encodedArgs = encodedConstructorArgs ? encodedConstructorArgs : await fetchConstructorParams(explorerAPIUrl, explorerAPIKey, contractAddress, compilerMetadata.abi, sdk.getProvider(), storage);
    const requestBody = {
...
async function isVerifiedOnEtherscan(contractAddress, chainId, explorerAPIUrl, explorerAPIKey) {
  ...
  invariant__default["default"](await isContractDeployed(contractAddress, provider), "Contract not deployed yet.");
...
async function fetchConstructorParams(explorerAPIUrl, explorerAPIKey, contractAddress, abi, provider, storage) {
  ...
  if (data && data.status === RequestStatus.OK && data.result[0] !== undefined) {
    ...
    try {
      ...
      ethers.ethers.utils.defaultAbiCoder.decode(contract.deploy.inputs, `0x${constructorArgs}`);
    } catch (e) {
...
async function fetchDeployBytecodeFromPublishedContractMetadata(contractAddress, provider, storage) {
  ...
  if (compilerMetaUri) {
    const pubmeta = await new ThirdwebSDK("polygon").getPublisher().resolvePublishMetadataFromCompilerMetadata(compilerMetaUri);
...
exports.ClaimEligibility = ClaimEligibility;
...
exports.approveErc20Allowance = approveErc20Allowance;
...
exports.cleanCurrencyAddress = cleanCurrencyAddress;
...
exports.fetchAndCachePublishedContractURI = fetchAndCachePublishedContractURI;
