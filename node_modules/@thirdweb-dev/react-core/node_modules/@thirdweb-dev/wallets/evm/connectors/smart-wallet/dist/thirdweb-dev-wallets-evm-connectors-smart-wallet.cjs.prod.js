...
var keys = require('../../../../dist/keys-ddd5d524.cjs.prod.js');
...
async function getUserOpHashV06(userOp, entryPoint, chainId) {
  ...
  let encoded = ethers.utils.defaultAbiCoder.encode([userOpType], [{
    ...hashedUserOp
  }]);
...
class SmartWalletConnector extends connector.Connector {
  constructor(config) {
    ...
    defineProperty._defineProperty(this, "accountApi", void 0);
  ...
  async initialize(personalWallet) {
    ...
    const originalProvider = sdk$1.getChainProvider(config.chain, {
      thirdwebApiKey: config.thirdwebApiKey || keys.DEFAULT_WALLET_API_KEY
    });
  ...
  async executeBatch(transactions) {
    ...
    const values = transactions.map(() => ethers.BigNumber.from(0)); // TODO check if we can handle multiple values
  ...
  async getChainSlug(chainOrRpc, provider) {
    ...
    if (typeof chainOrRpc === "number") {
      const chain = chains.getChainByChainId(chainOrRpc);
      return chain.slug;
    }
    if (typeof chainOrRpc === "string") {
      if (chainOrRpc.startsWith("http") || chainOrRpc.startsWith("ws")) {
        ...
        const chain = chains.getChainByChainId(chainId);
        return chain.slug;
      }
