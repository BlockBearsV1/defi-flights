...
import { getChainRPCs, defaultChains, getChainRPC } from '@thirdweb-dev/chains';
import { createAsyncLocalStorage, assertWindowEthereum } from '@thirdweb-dev/wallets';
...
import { UserWallet, fetchCurrencyMetadata, fetchCurrencyValue, getCachedAbiForContract, ListingType, ThirdwebSDK as ThirdwebSDK$1 } from '@thirdweb-dev/sdk';
...
function ThirdwebWalletProvider(props) {
  ...
  const handleWalletConnect = useCallback(async function (wallet, connectParams) {
  ...
  useEffect(() => {
    ...
    async function autoconnect() {
      ...
      if (personalWalletInfo) {
        const personalWalleObj = props.supportedWallets.find(W => W.id === personalWalletInfo.walletId);
        if (personalWalleObj) {
...
function useIsConnecting() {
  const context = useThirdwebWallet();
  invariant(context, "useConnectionStatus() must be used within a <ThirdwebProvider/>");
  return context.connectionStatus === "connecting" || context.connectionStatus === "unknown";
}
...
function useNetwork() {
  ...
  const switchChainNotSupported = !wallet || wallet.walletId === "walletConnectV1" || wallet.walletId === "walletConnect" || wallet.walletId === "metamask" && !isMetamaskInstalled || wallet.walletId === "coinbaseWallet" && !isCoinbaseInstalled;
...
async function doLogin(wallet, options) {
  ...
  const payloadData = {
    ...
    expiration_time: new Date(options?.expirationTime || Date.now() + 1000 * 60 * 5).toISOString(),
    invalid_before: new Date(options?.invalidBefore || Date.now()).toISOString(),
...
export { useUnclaimedNFTSupply as $, useClaimConditions as A, useClaimIneligibilityReasons as B, useActiveClaimConditionForWallet as C, useSetClaimConditions as D, useResetClaimConditions as E, usePrimarySaleRecipient as F, useUpdatePrimarySaleRecipient as G, useRoyaltySettings as H, useUpdateRoyaltySettings as I, usePlatformFees as J, useUpdatePlatformFees as K, useMetadata as L, useUpdateMetadata as M, useContractType as N, contractType as O, useCompilerMetadata as P, compilerMetadata as Q, useContract as R, useContractMetadata as S, ThirdwebThemeContext as T, useContractMetadataUpdate as U, useContractEvents as V, useContractRead as W, useContractWrite as X, useUnclaimedNFTs as Y, useClaimedNFTs as Z, __DEV__ as _, resolveMimeType as a, useEditionDrop as a$, useClaimedNFTSupply as a0, useBatchesToReveal as a1, useClaimNFT as a2, useLazyMint as a3, useDelayedRevealLazyMint as a4, useRevealLazyMint as a5, useListing as a6, useDirectListing as a7, useEnglishAuction as a8, useListings as a9, useTotalCount as aA, useTotalCirculatingSupply as aB, useOwnedNFTs as aC, useNFTBalance as aD, useMintNFT as aE, useMintNFTSupply as aF, useTransferNFT as aG, useAirdropNFT as aH, useBurnNFT as aI, useAllRoleMembers as aJ, useRoleMembers as aK, useIsAddressRole as aL, useSetAllRoleMembers as aM, useGrantRole as aN, useRevokeRole as aO, useTokenSupply as aP, useTokenBalance as aQ, useTokenDecimals as aR, useMintToken as aS, useClaimToken as aT, useTransferToken as aU, useTransferBatchToken as aV, useBurnToken as aW, useLogin as aX, useLogout as aY, useUser as aZ, useAuth as a_, useDirectListings as aa, useValidDirectListings as ab, useEnglishAuctions as ac, useValidEnglishAuctions as ad, useListingsCount as ae, useDirectListingsCount as af, useEnglishAuctionsCount as ag, useActiveListings as ah, useWinningBid as ai, useEnglishAuctionWinningBid as aj, useAuctionWinner as ak, useBidBuffer as al, useMinimumNextBid as am, useCreateDirectListing as an, useCreateAuctionListing as ao, useCancelListing as ap, useCancelDirectListing as aq, useCancelEnglishAuction as ar, useMakeBid as as, useMakeOffer as at, useAcceptDirectListingOffer as au, useExecuteAuctionSale as av, useOffers as aw, useBuyNow as ax, useNFT as ay, useNFTs as az, ThirdwebProviderCore as b, useEdition as b0, useNFTDrop as b1, useMarketplace as b2, useNFTCollection as b3, usePack as b4, useToken as b5, useTokenDrop as b6, useVote as b7, useSplit as b8, useMultiwrap as b9, useSignatureDrop as ba, useStorageUpload as bb, useStorage as bc, useNetworkMismatch as bd, useReadonlySDK as be, useSigner as bf, useSupportedChains as bg, useSupportedWallet as bh, useAppURI as bi, useSetAppURI as bj, ThirdwebSDKProvider as bk, useSDK as bl, useSDKChainId as bm, getErcs as bn, getErc1155 as bo, getErc721 as bp, getErc20 as bq, useWatchTransactions as br, useConnect as c, useConnectionStatus as d, useCreateWalletInstance as e, useDisconnect as f, useSwitchChain as g, useWallets as h, useIsConnecting as i, useNetwork as j, useThirdwebWallet as k, useBalance as l, useConnectedWallet as m, useAddress as n, useChainId as o, useActiveChain as p, ThirdwebAuthProvider as q, resolveIpfsUri as r, useThirdwebAuthContext as s, ThirdwebConfigProvider as t, useWallet as u, useThirdwebConfigContext as v, ThirdwebConnectedWalletProvider as w, useThirdwebConnectedWalletContext as x, useActiveClaimCondition as y, useClaimerProofs as z };
